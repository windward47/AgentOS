...
**Agent 核心的实现：** 通过 RPC 子进程 (`omp --mode rpc`) 接入 [oh-my-pi](https://github.com/can1357/oh-my-pi)，复用其 LLM 编排、工具调用、对话管理能力。Companion 不重复实现 Agent 逻辑，而是做 omp 的"前端"——提供语音交互和虚拟形象呈现。
...
- **主要方案（推荐）：** 通过子进程接入 **oh-my-pi** (`omp --mode rpc`)。  
  oh-my-pi 提供完整的 LLM 编排、32+ 内置工具、多模型路由、流干预规则。  
  Companion 通过 RPC (NDJSON over stdio) 与 omp 通信，专注于语音/感知/呈现。
...
| 项目 | 优势 | 集成方式 |
|------|------|----------|
| **oh-my-pi** ✅ | 32+ 内置工具、40+ LLM 提供商、RPC 模式、MIT 协议 | `omp --mode rpc` （NDJSON over stdio 子进程） |
...
| Agent 后端 | **oh-my-pi** (`omp --mode rpc`) | RPC 子进程，NDJSON 通信 |

...
| 项目 | 说明 |
|------|------|
| 渲染引擎 | Tauri WebView + PixiJS + pixi-live2d-display |
| 形象格式 | Live2D (.model3.json) |
| 形象驱动 | 嘴型（音频振幅 → ParamMouthOpenY）+ 情绪（wav2vec2 → 表情切换） |
| 状态动画 | idle / speaking / listening / thinking / error |
| 默认模型 | Hiyori 或 Epsilon 开源模型 |
...
- 语言：Rust（可与 Tauri 后端共享代码）
- 可选实现：使用 OpenCode、Oh My Pi 等成熟项目作为后端，通过 MCP 协议调用
...
| 模块 | 本地方案 | 云端方案 | 接口标准 |
|------|----------|----------|----------|
| ASR (语音→文本) | Whisper.cpp (FFI/子进程) | OpenAI Whisper API / Azure | Rust trait: `fn process_audio(chunk: &[f32]) -> Option<String>` |
| TTS (文本→语音) | ChatTTS (Python HTTP 服务) | Azure TTS / ElevenLabs | Rust trait: `fn synthesize(text: &str) -> Vec<f32>` |
...
### 4.1 推荐策略：复用 Agent 后端

不用从零实现 LLM 编排和工具调用。推荐两种选择：

| 项目 | 优势 | 集成方式 |
|------|------|----------|
| **OpenCode** | 免费模型策略、MIT 协议、MCP 服务模式 | `opencode serve` → Tauri 通过 MCP 客户端调用 |
| **Oh My Pi** | Rust 编写、性能好、与技术栈一致 | 嵌入为 Rust crate 或子进程通信 |

**架构示意（以 OpenCode 为例）：**

```
┌─ 你的应用 (Tauri) ──────────────────────┐
│  Live2D 界面 │ 语音模块 │ MCP 客户端    │
└──────────────────────┬──────────────────┘
                       │ MCP (JSON-RPC)
┌──────────────────────▼──────────────────┐
│  OpenCode serve (独立进程)                │
...
| Agent 后端 | OpenCode / Oh My Pi (MCP 协议) | 白嫖成熟能力 |
| ASR | Whisper.cpp / OpenAI Whisper API | 可切换 |
| TTS | ChatTTS / Azure TTS / ElevenLabs | 可切换 |
| LLM | Ollama (本地) / GPT-4o / Claude | 可切换 |
| VAD | webrtcvad / silero-vad | — |
| 情绪识别 | wav2vec2-base 微调 | 本地运行 |
| 工具协议 | MCP (JSON-RPC over stdio/WebSocket) | 标准化 |
| 插件扩展 | Wasm 沙盒 | 社区贡献 |
| 通信 | WebSocket / MQTT（可选） | 跨进程/跨设备 |

...
onMounted(async () => {
  ...
  config.value = {
    ...
    global_voice: {
      ...
      inject_mode: 'keyboard', asr_engine: 'mimo', tts_engine: 'mimo-tts', tts_provider: 'xiaomi', tts_voice: '茉莉', tts_speed: 1.0,
    },
  }
    ...
    <div v-if="config" class="max-w-2xl mx-auto w-full px-6 py-6 space-y-6">
      ...
      <section class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="p-5 grid grid-cols-2 gap-4">
          ...
          <div><label class="block text-[11px] font-medium text-gray-600 mb-1">TTS Provider</label>
            <select v-model="config.global_voice.tts_provider" class="block w-full rounded-lg border border-gray-200 px-3 py-2 text-sm">
              <option value="xiaomi">Xiaomi</option>
          ...
          <div><label class="block text-[11px] font-medium text-gray-600 mb-1">TTS Voice</label>
            <select v-model="config.global_voice.tts_voice" class="block w-full rounded-lg border border-gray-200 px-3 py-2 text-sm">
              <option v-for="v in (TTS_VOICES[config.global_voice.tts_provider] || TTS_VOICES['xiaomi'])" :key="v" :value="v">{{ v }}</option>
            </select>
          </div>
